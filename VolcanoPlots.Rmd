...
volcano_plot<-function(filtered_result){
  keyvals <- ifelse(
    filtered_result$hgnc_symbol %in% gene_sig_29$Gene.Symbol, 'orange3',
    ifelse(filtered_result$pvalue <= 0.01, 'steelblue3', 
           'grey')
  )
  ...
  p1 <- EnhancedVolcano(filtered_result,
                        ...
                        FCcutoff = 0,
                        pointSize = 1.0,
                        selectLab = setdiff(unique(filtered_result$hgnc_symbol),NA),
                        labSize = 4.0,
                        labCol = 'black',
                        boxedLabels = FALSE,
