...
library(tidyr)
library(tidyverse)
Palette1<-c("#141C52","#F23F1C","#832E37","gray")
names(Palette1)<-c("ABMR","TCMR","Mixed ABMR/TCMR","NoReject")
...
```{r}
ggp<-ggplot(Long, aes(x = name, y = value, color = Group2)) +
  geom_boxplot(outlier.shape = NA) +
  geom_point(position = position_jitterdodge(dodge.width = 0.7, jitter.width = 0.1), 
             size = 0.05, alpha = 0.5) +
...
ggsave(plot = ggp,filename="/Users/alinaklineschoder/Desktop/TCMRvABMRvNR.png",width = 10,height = 10)

ggp<-ggplot(Long%>%filter(Group2%in%c("ABMR","TCMR","Mixed ABMR/TCMR")), aes(x = name, y = value, color = Group2)) +
  geom_boxplot(outlier.shape = NA) +
  geom_point(position = position_jitterdodge(dodge.width = 0.7, jitter.width = 0.1), 
             size = 0.05, alpha = 0.5) +
  ggpubr::stat_compare_means(method = "anova", label = "p.format", 
...
ggsave(plot = ggp,filename="/Users/alinaklineschoder/Desktop/TCMRvABMR.png",width = 10,height = 8)
```
...
ggp<-ggplot(Long,aes(x=name,y=value,color=reject))+geom_boxplot(outlier.shape = NA)+
  geom_point(position=position_jitterdodge(dodge.width=0.7,jitter.width = 0.1),size=0.05,alpha=0.5)+
  ggpubr::stat_compare_means(method="t.test",label = "p.format",hide.ns = F,size=3,hjust=0.5,vjust=0.5) + theme(legend.position = "top")+coord_flip()+theme_bw()+scale_color_manual(values=Palette2)+theme(legend.title = element_blank())+theme(strip.background = element_rect(fill="white"),strip.text.y.right = element_text(angle=0),panel.grid = element_blank())+labs(y="Composition (Min/Max Normalized)")+theme(axis.title.y = element_blank())

ggsave(plot = ggp,filename="/Users/alinaklineschoder/Desktop/RvNR_Broad.png",width = 10,height = 6)

```

```{r}

ggp<-ggplot(Long,aes(x=name,y=value,color=Group2))+geom_boxplot(outlier.shape = NA)+
  geom_point(position=position_jitterdodge(dodge.width=0.7,jitter.width = 0.1),size=0.05,alpha=0.5)+
  ggpubr::stat_compare_means(method="anova",label = "p.format",hide.ns = F,size=3,hjust=0.5,vjust=0.5) + theme(legend.position = "top")+coord_flip()+theme_bw()+scale_color_manual(values=Palette1)+theme(legend.title = element_blank())+theme(strip.background = element_rect(fill="white"),strip.text.y.right = element_text(angle=0),panel.grid = element_blank())+labs(y="Composition (Min/Max Normalized)")+theme(axis.title.y = element_blank(),legend.position = "top")

ggsave(plot = ggp,filename="/Users/alinaklineschoder/Desktop/ABMRvTCMRvNR_Broad.png",width = 10,height = 6)

ggp<-ggplot(Long%>%filter(Group2%in%c("ABMR","TCMR","Mixed ABMR/TCMR")),aes(x=name,y=value,color=Group2))+geom_boxplot(outlier.shape = NA)+
  geom_point(position=position_jitterdodge(dodge.width=0.7,jitter.width = 0.1),size=0.05,alpha=0.5)+
  ggpubr::stat_compare_means(method="anova",label = "p.format",hide.ns = F,size=3,hjust=0.5,vjust=0.5) + theme(legend.position = "top")+coord_flip()+theme_bw()+scale_color_manual(values=Palette1)+theme(legend.title = element_blank())+theme(strip.background = element_rect(fill="white"),strip.text.y.right = element_text(angle=0),panel.grid = element_blank())+labs(y="Composition (Min/Max Normalized)")+theme(axis.title.y = element_blank(),legend.position = "top")

ggsave(plot = ggp,filename="/Users/alinaklineschoder/Desktop/ABMRvTCMR_Broad.png",width = 10,height = 6)
