...
ggplot(Long, aes(x = name, y = value, color = reject)) +
  ...
  coord_flip() +
  theme_bw() +
  scale_color_manual(values = c("#141C52", "#F23F1C","#DA188D")) +
...
```{r}
ggplot(Long, aes(x = name, y = value, color = Group2)) +
  geom_boxplot(outlier.shape = NA) +
  geom_point(position = position_jitterdodge(dodge.width = 0.7, jitter.width = 0.1), 
             size = 0.05, alpha = 0.5) +
  ...
  coord_flip() +
  theme_bw() +
  scale_color_manual(values = c("#141C52","#F23F1C","gray")) +
  theme(legend.title = element_blank()) +
  facet_grid(Broad ~ ., scales = "free_y", drop = TRUE, space = "free") +
  theme(strip.background = element_rect(fill = "white"),
        strip.text.y.right = element_text(angle = 0),
...
```{r}
ggplot(Long,aes(x=name,y=value,color=Group2))+geom_boxplot(outlier.shape = NA)+
  geom_point(position=position_jitterdodge(dodge.width=0.7,jitter.width = 0.1),size=0.05,alpha=0.5)+
  ggpubr::stat_compare_means(method="anova",label = "p.format",hide.ns = F,size=3,hjust=0.5,vjust=0.5) + theme(legend.position = "top")+coord_flip()+theme_bw()+scale_color_manual(values=c("#141C52","#F23F1C","gray"))+theme(legend.title = element_blank())+theme(strip.background = element_rect(fill="white"),strip.text.y.right = element_text(angle=0),panel.grid = element_blank())+labs(y="Composition (Min/Max Normalized)")+theme(axis.title.y = element_blank(),legend.position = "top")
